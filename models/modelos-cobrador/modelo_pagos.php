...
function alta_de_pago($formulario_pago)
	{
		...
		$fecha_ideal_de_pago = $formulario_pago['fecha_ideal_de_pago'];
		$id_usuario_que_cobro = $formulario_pago['id_usuario_que_cobro'];
		$fecha_de_pago_credito = $formulario_pago['fecha_de_pago_credito'];
/*
		$buscar_primer_pago = $this->db->query('SELECT id_pago_creditos,fecha_ideal_de_pago FROM tabla_pago_creditos where monto_de_pago_credito = 0 and id_credito ="'.$id_credito.'"');
		if ($buscar_primer_pago->num_rows() > 0){
			foreach ($buscar_primer_pago->result() as $row){
